/**
 * Misc stateless message handlers (audit P2-3 split).
 *
 * The leftover handlers that did not belong to any of the family modules now
 * behind the ./index barrel — extracted verbatim so ./index is a pure re-export
 * barrel. Covers: model_changed, claude_ready, agent_idle / agent_busy,
 * thinking_level_changed, the slash_commands / agent_list / provider_list /
 * file_list list-replacement family, available_models, mcp_servers,
 * session_cost_threshold_crossed, notification_prefs, and the #554
 * permission-request stream-split resolver. No side effects — see the
 * per-function docs. Re-exported through ./index so the public surface is
 * unchanged.
 */
