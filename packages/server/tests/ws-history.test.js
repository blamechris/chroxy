...
describe('replayHistory', () => {
  ...
  // #4638: a session persisted while a stream was in flight (claude TUI
  // emitted stream_start then nothing — the v0.9.21 wedge mode) ends
  // with a stream_start that never closed. Without this heal the
  // dashboard's streamingMessageId/Working banner stays set after
  // reconnect until next chroxy restart. Synthesizes stream_end +
  // agent_idle from the dangling messageId; history itself is not
  // mutated (same pattern as the #4628 fan-out above).
  it('emits synthetic stream_end + agent_idle when history ends on a dangling stream_start (#4638)', async () => {
    const history = [
      { type: 'message', text: 'do the thing', role: 'user' },
      { type: 'stream_start', messageId: 'msg-stuck' },
      // No stream_end, no result — the wedge.
    ]
    const { manager } = createMockSessionManager([
      { id: 'sess-1', name: 'Alpha', cwd: '/alpha' },
    ])
    ...
    assert.deepEqual(types, [
      ...
      'agent_idle',
      'history_replay_end',
    ], 'dangling stream_start gets synthetic stream_end + agent_idle')

    const streamEnd = ctx._sends.find(m => m.type === 'stream_end')
    assert.equal(streamEnd.messageId, 'msg-stuck', 'synthetic stream_end carries the dangling messageId')
    assert.equal(streamEnd.sessionId, 'sess-1', 'and the session id')

    const agentIdle = ctx._sends.find(m => m.type === 'agent_idle')
    assert.equal(agentIdle.sessionId, 'sess-1', 'agent_idle carries the session id')
  })

  it('does not emit synthetic stream_end when stream_start was closed by stream_end (#4638)', async () => {
    const history = [
      { type: 'stream_start', messageId: 'msg-closed' },
      { type: 'stream_end', messageId: 'msg-closed' },
    ]
    const { manager } = createMockSessionManager([
      { id: 'sess-1', name: 'Alpha', cwd: '/alpha' },
    ])
    ...
    const streamEndCount = ctx._sends.filter(m => m.type === 'stream_end').length
    assert.equal(streamEndCount, 1, 'only the real stream_end, no synthetic')
    const types = ctx._sends.map(m => m.type)
    assert.ok(!types.includes('agent_idle'), 'closed stream → no synthetic agent_idle either')
  })

  it('does not emit synthetic stream_end when stream_start was closed by result (#4638)', async () => {
    const history = [
      { type: 'stream_start', messageId: 'msg-r' },
      { type: 'result', cost: null, duration: 50 },
    ]
    const { manager } = createMockSessionManager([
      { id: 'sess-1', name: 'Alpha', cwd: '/alpha' },
    ])
    ...
    const streamEndCount = ctx._sends.filter(m => m.type === 'stream_end').length
    assert.equal(streamEndCount, 0, 'no synthetic stream_end when result closed the turn')
    const agentIdleCount = ctx._sends.filter(m => m.type === 'agent_idle').length
    assert.equal(agentIdleCount, 1, 'exactly one agent_idle (from the #4628 fan-out, not the #4638 heal)')
  })
})
