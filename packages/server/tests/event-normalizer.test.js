...
describe('EventNormalizer', () => {
  ...
  describe('result event occupancy snapshot (#6769)', () => {
    it('forwards contextUsage to the wire when the session emitted one', () => {
      const contextUsage = {
        totalTokens: 110_000,
        maxTokens: 200_000,
      ...
      const result = normalizer.normalize(
        'result',
        { cost: 0.01, duration: 5, usage: { input_tokens: 1 }, sessionId: 'sess-1', contextUsage },
        makeCtx(),
      )
      const resultMsg = result.messages.find((m) => m.msg.type === 'result')
      assert.deepEqual(resultMsg.msg.contextUsage, contextUsage)
    })

    it('omits contextUsage from the wire when the session did not emit one', () => {
      const result = normalizer.normalize(
        'result',
      ...
      const resultMsg = result.messages.find((m) => m.msg.type === 'result')
      assert.equal('contextUsage' in resultMsg.msg, false,
