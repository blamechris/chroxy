...
The server runs in **CLI headless mode**: it executes the Claude Agent SDK in-process by default, or shells out to `claude -p`, `gemini --prompt=<text>`, or `codex exec` for the other providers, and streams parsed events to clients. No tmux, no PTY.
