...
import { broadcastEnvironmentList } from './environments/authority.js'
 ...
 *   { type: 'session_pr_status', requestId?, sessionId, generatedAt, branch, repo, pr, checks, merge, reason } — the session's pull-request / CI status (#7344, display slice); reply to a `session_pr_status_request`. SESSION-scoped, not host-scoped: a pairing-bound token may ask about the session it is bound to and no other. Assembled from ONE `gh pr list` response (plus git for branch/remote) using the user's existing local `gh auth`, so `checks` and `merge` always describe the same head SHA (`pr.headRefOid`). `checks.state:'none'` means this head produced NO run and is NOT a green; `checks` and `merge` are separate facts and must never be collapsed into one "ready to merge?" badge (21/21 green alongside `mergeStateStatus:'BLOCKED'` is the case that motivated the issue). `pr:null` with `reason:null` is the quiet negative (no open PR for this branch) — on a fork checkout it is BEST-EFFORT, since a transiently-failed upstream widening reports the same shape (#7435; the distinction is held server-side for the CI watcher and not transmitted); `pr:null` with a `reason` means the survey could not determine it, and must never render as an implied green.
 *   { type: 'session_pr_threads', requestId?, sessionId, countedAt, prNumber, unresolvedCount, totalCount, truncated, reason } — the unresolved REVIEW-THREAD COUNT for the session's pull request (#7430, on-demand follow-on to #7344); reply to a `session_pr_threads_request`. Same SESSION-scoped authority as `session_pr_status`: a pairing-bound token may ask about the session it is bound to and no other. A SEPARATE message from `session_pr_status` on purpose — the count needs a GraphQL `reviewThreads` read that `gh pr list --json` cannot serve, and since #7426 that survey runs on a daemon-side sweep over every session, so folding the count in would put a second `gh` subprocess on every tick of a background poll for a field only a click wants. This pair is sent by the click and never by the sweep. `countedAt` is its OWN timestamp, not the status snapshot's `generatedAt` — the two go stale on different schedules. `unresolvedCount: 0` means the server COUNTED and found none; `unresolvedCount: null` (always with a `reason`) means it could not find out, and a client MUST render those differently — a missing count printed as "0 unresolved threads" reads as "nothing is blocking this PR", the exact false green this surface exists to prevent. `truncated: true` means more threads exist than one page read, so the count is a LOWER BOUND and a truncated 0 is not a definite zero. `prNumber` names the PR the count describes so a consumer can check it agrees with the PR its `session_pr_status` snapshot names.
 ...
 *   { type: 'environment_error', error, environmentId?, code?, sessions? } — environment operation error. `code` is 'DOCKER_IMAGE_NOT_ALLOWED' (image allowlist) or 'ENVIRONMENT_HAS_LIVE_SESSIONS' (#7562 destroy refusal); `sessions` accompanies the latter with the ids still running inside the environment
  ...
  constructor({ port, apiToken, cliSession, sessionManager, defaultSessionId, authRequired = true, pushManager = null, maxPayload, noEncrypt, keyExchangeTimeoutMs, localhostBypass, tokenManager, pairingManager, serverIdentity = null, maxPendingConnections, backpressureThreshold, environmentManager, orchestrationManager = null, schedulerEngine = null, sessionCiWatcher = null, config = null, diagnosticsRateLimit = null, devicePreferences = null, pagesStore = null, pagesRateLimiter } = {}) {
    ...
    if (this.environmentManager && typeof this.environmentManager.on === 'function') {
      this._environmentSessionsChangedHandler = () => {
        try {
          // #7596: fan the environment list to UNBOUND (host) clients only.
          // This is the most frequent leak path — it fires on every session
          // open/close — so a plain broadcast would stream the host-level
          // surface to every bound (share-a-session) listener continuously.
