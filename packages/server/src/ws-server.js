 ...
 *   { type: 'agent_completed', sessionId, toolUseId }    — that subagent reached a terminal state. #7340: emitted on the provider's own terminal signal (the CLI/SDK `task_notification`, byok's child returning, or a cancel), and at turn end for a FOREGROUND subagent the model abandoned mid-turn. A BACKGROUNDED subagent is deliberately NOT completed by turn end — it outlives the turn, and saying otherwise showed the session idle with reviewers still running.
