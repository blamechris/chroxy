...
const TOOLS = {
  ...
  chroxy_interrupt_session: {
    description: "Best-effort interrupt of a running session (the protocol defines no correlated ack for this). Only a session created by chroxy_create_session in this same MCP-server process can be interrupted — a session created elsewhere is refused with `status: 'rejected', reason: 'not_owned'`, before any network I/O. The result reports what was observed, not a synthesized \"stopped\".",
