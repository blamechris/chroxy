...
export function generateWindowsServiceWrapper(config) {
  ...
  // #7162: rotate oversized captures HERE, before the `>>` redirect opens a
  // handle on them. cmd's redirect handle lacks POSIX append semantics, so
  // the daemon cannot safely truncate these in place on Windows (a truncate
  // under the open handle NUL-pads the gap on the next write); a rename
  // before the handle exists is the safe equivalent. 5242880 = 5MB, matching
  // the POSIX-side STDIO_CAPTURE_MAX_SIZE.
  const rotateCapture = (live, old) =>
    `if exist "${live}" for %%A in ("${live}") do if %%~zA gtr 5242880 move /y "${live}" "${old}"`
  const lines = [
    '@echo off',
  ...
  for (const name of ['chroxy.log', 'chroxy-stdout.log', 'chroxy-stderr.log']) {
    ...
    try {
      ...
      for (const text of lines) {
        if (!text) continue
        // Plain format leads with the ISO stamp; JSON mode carries "ts".
        // Unstamped lines (crash traces, banners) inherit the preceding
        // stamped line's time so multi-line output stays together.
        const m = /^(\d{4}-\d{2}-\d{2}T[\d:.]+Z)/.exec(text)
          || /^\{"ts":"(\d{4}-[^"]+)"/.exec(text)
        if (m) {
          const parsed = Date.parse(m[1])
          if (!Number.isNaN(parsed)) last = parsed
        }
        tails.push({ ts: last, text })
      }
    } catch {
      // Unreadable candidate — surface whatever the other file has
    }
  }
  if (tails.length > 0) {
    // Array.prototype.sort is stable: equal timestamps keep push order.
    tails.sort((a, b) => a.ts - b.ts)
    result.recentLogs = tails.slice(-5).map(t => t.text)
  }
