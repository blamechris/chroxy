 ...
 * session state matches each fixture's expectation — the `messages` list, any
 * other session field the fixture names (#6325), the flat connection-state
 * slice, and the terminal/toast side-channels.
 *
    ...
    it(`${fx.name}`, () => {
      ...
      for (const [id, fields] of Object.entries(exp!.sessions ?? {})) {
        ...
        if (Object.keys(literals).length > 0) {
          expect(ss).toMatchObject(literals);
        }
      }
