...
describe("history_replay_end: '(resolved)' sweep vs a racing live AskUserQuestion (#7420)", () => {
  ...
  // contributes no baseline, so pre-fix the nested full start was the first to
  // set one and captured a `messages.length` that already counted the racer —
  // which the deferred swap then sliced straight out of `messages`. A drop, not
  // a stamp, exactly like #7477, reached from the other side.
  it('keeps every racer when a full rebuild nests inside a DELTA window (#7492)', () => {
    // A pre-replay message makes the outer baseline non-zero, so the final swap
    // is real (it drops the prefix) rather than an identity slice.
    const store = seedOne([{ id: 'old-1', type: 'response', content: 'before', timestamp: 1 }]);
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    // Racer #1 — inside the DELTA window and BEFORE the nested full start. This
    // is the one #7492 lost.
    _testMessageHandler.handle(question({ toolUseId: 'q-use-1' }));
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    ...
    expect(msgs).toHaveLength(3);
    // ...the swap really did happen (the pre-replay prefix is gone), so this is
    // not passing because the rebuild was cancelled altogether...
    expect(msgs.every((m: any) => m.type === 'prompt')).toBe(true);
    // ...and neither end's sweep stamped any of them.
    expect(answeredOf(store, 0)).toBeUndefined();
    expect(answeredOf(store, 1)).toBeUndefined();
  ...
  // #7519 — the ORDERING artifact the #7492 interleave left behind, at the wire
  // level. When the outermost DELTA has already APPENDED before the nested full
  // start, its entries sit inside the preserved tail; `replayDedupCache` is
  // scoped at the same baseline, so the full replay's re-delivery of them is
  // suppressed and they keep their early position while the OLDER history the
  // full replay delivers lands after them. Nothing lost, transcript mis-ordered.
  // Measured on the branch point through exactly these frames:
  // ['h-2','racer','h-1'] where ['h-1','h-2','racer'] is the truth.
  //
  // The fix is per-append provenance: `handleMessage` scopes each dispatched
  // frame (its `historySeq` is the replayed-vs-live discriminator) and
  // `updateSession` reports every session-messages write, so the swap returns
  // (replayed, in the server's history order) ++ (live, in arrival order).
  it('swaps in HISTORY order when the delta appended before the nested full start (#7519)', () => {
    const store = seedOne([{ id: 'old-1', type: 'response', content: 'before', timestamp: 1 }]);
    ...
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    _testMessageHandler.handle(entry('h-2', { historySeq: 2 })); // the delta replays the newest entry
    _testMessageHandler.handle(entry('racer')); // a LIVE broadcast — no historySeq
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    _testMessageHandler.handle(entry('h-1', { historySeq: 1 })); // full replay, oldest first
    _testMessageHandler.handle(entry('h-2', { historySeq: 2 })); // ...re-delivered, deduped
    // The array really is in the mis-ordered shape before the swap resolves it —
    // otherwise this passes on a fixture that never took effect.
    expect(store.getState().sessionStates.s1.messages.map((m: any) => m.id)).toEqual([
      'old-1',
      'h-2',
      'racer',
      'h-1',
    ...
    const msgs = store.getState().sessionStates.s1.messages;
    expect(msgs.map((m: any) => m.id).slice().sort()).toEqual(['h-1', 'h-2', 'racer']);
    expect(msgs.map((m: any) => m.id)).toEqual(['h-1', 'h-2', 'racer']);
    expect(getReplayWindowDepth('s1')).toBe(0);
  });

  it('the observation point covers the SHARED dispatch table too, not just `case message` (#7519)', () => {
    // The hook is on `updateSession`, the one funnel every session-messages
    // write goes through — including store-core's dispatch table, which reaches
    // it via the store adapter. Same interleave, but the live racer arrives as a
    // `user_question` (a different handler in a different package) and is still
    // classified live and still ordered last.
    const store = seedOne([{ id: 'old-1', type: 'response', content: 'before', timestamp: 1 }]);
    const entry = (messageId: string, extra: Record<string, unknown> = {}) => ({
      type: 'message',
      messageType: 'response',
    ...
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    _testMessageHandler.handle(entry('h-2', { historySeq: 2 }));
    _testMessageHandler.handle(question({ toolUseId: 'q-live' })); // LIVE, via the dispatch table
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    _testMessageHandler.handle(entry('h-1', { historySeq: 1 }));
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });

    const msgs = store.getState().sessionStates.s1.messages;
    expect(msgs.map((m: any) => m.type)).toEqual(['response', 'response', 'prompt']);
  ...
  it('keeps replay order when a user types and a thinking placeholder is stripped mid-replay (#7577)', () => {
    ...
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    _testMessageHandler.handle(entry('h-2', { historySeq: 2 }));
    _testMessageHandler.handle(entry('racer'));
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    // `sendMessage`'s exact optimistic write (connection.ts:1710) — user bubble
  ...
  it('uses an extra message update only when a thinking placeholder exists (#7577)', () => {
    let store = seedOne([{ id: 'thinking', type: 'thinking', content: '', timestamp: 1 }]);
    const withPlaceholder = jest.spyOn(store, 'setState');
    _testMessageHandler.handle({
      type: 'message',
      messageType: 'response',
      content: 'first',
    ...
    expect(store.getState().sessionStates.s1.messages.map((m: any) => m.id)).toEqual(['first']);
    withPlaceholder.mockRestore();

    store = seedOne();
    const withoutPlaceholder = jest.spyOn(store, 'setState');
    _testMessageHandler.handle({
      type: 'message',
      messageType: 'response',
      content: 'second',
    ...
    expect(store.getState().sessionStates.s1.messages.map((m: any) => m.id)).toEqual(['second']);
  });
  ...
  // `sendInterrupt` (connection.ts:1876) removes every queued bubble without
  // going anywhere near the replay window — so the swap cut two entries INTO the
  // replayed tail and the store kept only the last one. Measured pre-fix: 1
  // message where 3 were replayed.
  it('keeps the replayed set when Stop drops queued bubbles mid-window (#7524)', () => {
    ...
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    _testMessageHandler.handle(question({ toolUseId: 'r-1' }));
    _testMessageHandler.handle(question({ toolUseId: 'r-2' }));
    _testMessageHandler.handle(question({ toolUseId: 'r-3' }));
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });
...
// in the ring buffer), which means it carries `historySeq` and is correctly
// classified as replay-delivered by #7420's discriminator — and the sweep then
// stamps it '(resolved)'. Background the phone mid-question, reopen it, and the
// prompt is a dead pill with the agent still blocked on the resolver until it
// times out.
//
// The server closes it: `resendPendingQuestions` (ws-history.js) re-sends every
// question a session is still blocked on as a LIVE frame from `finishReplay`,
// AFTER the end frame, so the sweep has already run when it lands. The client's
// part is that a live re-delivery SUPERSEDES the copy it matches by `toolUseId`
// — `answered` cleared, id and position kept — instead of stacking a second
// bubble under the resolved one. That is the same merge `handlePermissionRequest`
// has always done for a re-sent `permission_request`.
describe('a still-PENDING question survives an ordinary reconnect (#7457)', () => {
  ...
  const messagesOf = (store: any) => store.getState().sessionStates.s1.messages as any[];
  ...
  it('delta replay: the re-sent pending question is answerable again', () => {
    ...
    expect(messagesOf(store)[0].answered).toBe('(resolved)');

    // The server's post-replay re-send: same toolUseId, NO historySeq.
    _testMessageHandler.handle(question());

    expect(messagesOf(store)).toHaveLength(1);
    expect(messagesOf(store)[0].answered).toBeUndefined();
    expect(messagesOf(store)[0].toolUseId).toBe('ask-1');
  });
  ...
  it('full rebuild (session switch): the re-sent pending question is answerable again', () => {
    ...
    const store = seed([{ id: 'old-1', type: 'response', content: 'before', timestamp: 1 }]);
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: true });
    _testMessageHandler.handle(question({ historySeq: 3, timestamp: 1000 }));
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });
    expect(messagesOf(store)).toHaveLength(1);
  ...
  // The realistic reconnect for a client that stayed alive: it already holds the
  // question it saw LIVE before the socket dropped, and the full rebuild
  // re-delivers that same toolUseId out of the ring buffer. The replayed copy
  // must APPEND into the post-baseline tail — merged into the held copy instead
  // it would sit in the prefix that `messages.slice(base)` discards, and the
  // prompt would vanish from the transcript entirely (#7457's worse half). Then
  // the re-send revives the surviving copy.
  ...
  // bottom of the transcript, below turns that came after it. A delta replay is
  // append-only (no atomic swap), so index 1 here is a real position.
  it('the revived prompt keeps its place in the transcript', () => {
    const store = seed([{ id: 'old-1', type: 'response', content: 'before', timestamp: 1 }]);
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    _testMessageHandler.handle(question({ historySeq: 3, timestamp: 1000 }));
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });
    expect(messagesOf(store)).toHaveLength(2);
    expect(messagesOf(store)[1].answered).toBe('(resolved)');

    _testMessageHandler.handle(question());

    expect(messagesOf(store)).toHaveLength(2);
    expect(messagesOf(store)[0].id).toBe('old-1');
    expect(messagesOf(store)[1].type).toBe('prompt');
    expect(messagesOf(store)[1].answered).toBeUndefined();
  });

  // AC (b), unchanged: no re-send means the server said nothing is pending, so
  // a long-answered question replayed out of history keeps its stamp. Green on
  // BOTH sides of the fix and stated as such — it exists to catch an over-broad
  // fix, one that un-stamped on any replayed question.
  ...
  //
  // Reachable without any overlapping-replay exotica: a provider re-emitting the
  // same `AskUserQuestion` payload (the pre-#4668 failure mode) produces a live
  // frame for a `toolUseId` the client already holds, and if the held copy is in
  // the prefix the merge writes into a message `messages.slice(base)` then throws
  // away. The prompt is not stamped and not duplicated — it is GONE. `main` does
  // not have that failure (there the live frame appends into the tail and is
  // protected by the live-arrival ledger), so it would be a regression of
  // #7420's guarantee, not merely an uncovered corner.
  it('a live re-delivery during a full rebuild never deletes the held prompt (#7508 F2)', () => {
    ...
    expect(messagesOf(store)).toHaveLength(1);
    expect(messagesOf(store)[0].type).toBe('prompt');
    expect(messagesOf(store)[0].toolUseId).toBe('ask-1');
    expect(messagesOf(store)[0].answered).toBeUndefined();
  });

  // POSITIVE CONTROL for the bound above. A held copy INSIDE the rebuild tail is
  // still superseded — otherwise the fix could be "never supersede during a
  ...
  // clear the sweep's `(resolved)` placeholder — that IS #7457's bug — but not a
  // real decision: a second device can answer after the server's pending read and
  // before the frame lands, and nothing on the wire un-sticks a prompt revived on
  // top of a real answer.
  it('a re-delivery does not erase a real answer from another device', () => {
    ...
    expect(messagesOf(store)).toHaveLength(1);
    expect(messagesOf(store)[0].answered).toBe('Option A');
  });
  ...
  it('a re-send for one question does not collapse a second, unrelated one', () => {
    ...
    _testMessageHandler.handle({ type: 'history_replay_start', sessionId: 's1', fullHistory: false });
    _testMessageHandler.handle(question({ historySeq: 3, timestamp: 1000 }));
    _testMessageHandler.handle(question({ toolUseId: 'ask-2', historySeq: 4, timestamp: 1001 }));
    _testMessageHandler.handle({ type: 'history_replay_end', sessionId: 's1' });

    _testMessageHandler.handle(question({ toolUseId: 'ask-2' }));
