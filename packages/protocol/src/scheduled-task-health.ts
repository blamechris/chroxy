...
export interface ScheduledTaskHealthInput {
  ...
  readonly lastRun?: { readonly status?: string | null } | null
}
 ...
 * ordering prevents.
 */
export function deriveScheduledTaskHealthTag(
  task: ScheduledTaskHealthInput | null | undefined,
): ScheduledTaskHealthTag {
  if (!task || typeof task !== 'object') return 'ERROR'
  if (task.enabled === false) return 'PAUSED'
  const lastRun = task.lastRun
  if (!lastRun || typeof lastRun !== 'object') return 'NEVER RUN'
