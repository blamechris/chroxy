 ...
 */
export const McpServerSourceSchema = z.enum(['local', 'project-mcp-json', 'user'])
 ...
 */
export const McpTrustServerInputSchema = z.union([
  z.object({
    name: z.string(),
    url: z.string(),
...
export function buildPermissionRequestMessage(fields: {
  ...
  input: unknown
  // #7968: REQUIRED for the same reason as `input` above — see the doc comment.
  floored: boolean
  description?: string
  remainingMs?: number
  sessionId?: string
