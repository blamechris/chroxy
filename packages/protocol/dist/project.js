 ...
 */
function chroxyWorktreesRoot(env = defaultEnv()) {
 ...
 */
function chroxyWorktreeTopDir(dir, env = defaultEnv()) {
 ...
 */
export function worktreeParent(cwd, env = defaultEnv()) {
 ...
 */
export function classifyNonProjectCwd(cwd, env = defaultEnv()) {
    if (typeof cwd !== 'string' || cwd.length === 0)
        return null;
    const dir = realResolve(cwd);
    if (!dir)
    ...
    if (homeRaw) {
        ...
        if (dir === homeResolved || dir === realResolve(homeRaw))
            return 'home';
    }
    return null;
}
 ...
 */
export function deriveProject(cwd, env = defaultEnv()) {
 ...
 */
export function deriveProjectFromCwd(cwd, env = defaultEnv()) {
    if (typeof cwd !== 'string' || cwd.length === 0)
        return null;
    let dir;
    try {
