 ...
 */
export const LEGACY_THINKING_LEVELS = Object.freeze(['default', 'high', 'max']);
/** The level assumed when a message carries none, and the legacy fallback's default. */
export const LEGACY_DEFAULT_THINKING_LEVEL = 'default';
/**
 ...
 * unbounded STRING.
 */
export const THINKING_LEVEL_MAX_LENGTH = 32;
/**
 * The permitted charset. Deliberately narrow: every level observed on any
 * provider is a lowercase word, optionally hyphen/underscore separated. It
 * rejects path traversal (`../../etc`), whitespace, quotes and every shell or
 * JSON metacharacter, which is what keeps "the roster is open" from meaning
 * "anything at all reaches a subprocess param".
 */
const THINKING_LEVEL_PATTERN = /^[A-Za-z0-9_-]+$/;
/**
 ...
 */
export function isWellFormedThinkingLevel(value) {
    return typeof value === 'string'
        && value.length > 0
        && value.length <= THINKING_LEVEL_MAX_LENGTH
        && THINKING_LEVEL_PATTERN.test(value);
}
/**
 ...
 */
export function resolveThinkingLevels(row, opts) {
    const advertised = Array.isArray(row?.reasoningLevels)
        ? row.reasoningLevels.filter((l) => isWellFormedThinkingLevel(l))
        : [];
    ...
    const preferred = row?.defaultReasoningLevel;
    const defaultLevel = (isWellFormedThinkingLevel(preferred) && advertised.includes(preferred))
        ? preferred
        : advertised[0];
    return { levels: advertised, defaultLevel, source: 'model' };
}
/**
...
export function formatThinkingLevelLabel(level) {
    if (level === LEGACY_DEFAULT_THINKING_LEVEL)
        return 'Auto';
 ...
 */
export function thinkingLevelOptions(row, opts) {
