...
const PLATFORM_SPECIFIC = {
  ...
  'session_pr_status': 'dashboard', // #7344 session PR/CI status chip — dashboard-only for the display slice (the mobile app header has no equivalent chip yet); mobile parity is #7425
  'session_pr_threads': 'dashboard', // #7430 unresolved review-thread count for the CI prefill line — dashboard-only, because the surface that consumes it (the CI chip's prefill action, #7431) is dashboard-only; mobile has no CI chip at all, so parity rides with #7425 rather than being a separate gap
