...
describe('App', () => {
  ...
  // #4630 — every interactive header element exposes BOTH `title` (browser
  // hover tooltip) and `aria-label` (screen-reader announcement). Native
  // `title` tooltips render in Chrome/Safari/Firefox; aria-label is the
  // baseline accessible name regardless of how the visual tooltip ends up
  // rendered. The status dot and version badge were originally bare spans
  // with no discoverable label — hovering produced nothing, and SR users
  // had no way to know what they meant. Pinning the contract here so a
  // future refactor can't silently drop either attribute.
