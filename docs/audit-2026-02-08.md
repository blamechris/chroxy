...
**6. Unused dependency**
- `react-native-webview` is still in `package.json` but not used in code (removed in PR #71)
- Intentionally kept for planned xterm.js terminal integration
...
| **Terminal view with xterm.js** | **Not Started** | Biggest UX gap; xterm.js-based terminal not yet implemented. |
...
6. **xterm.js terminal integration** — This is the single biggest UX gap. Plan:
   - Confirm `react-native-webview` remains as a dependency and wire it up for the terminal WebView (already present in `packages/app/package.json`)
