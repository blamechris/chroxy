...
- **Default-off while a tunnel is active.** Whenever a tunnel is running (`tunnelActive` — a Quick Tunnel or a set tunnel URL), an unknown edge *could* be in front, so the loopback bypass is disabled and even a same-host dashboard does the (cheap, already-supported) key exchange. With **no** tunnel there is no unknown edge, so the fast loopback bypass still applies.
