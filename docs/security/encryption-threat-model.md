...
1. **Long-lived identity key.** On first run the daemon mints an **Ed25519** signing keypair (`nacl.sign`, already part of the `tweetnacl` dependency) and persists it across restarts — in the OS keychain under a dedicated service (`chroxy-identity-key`), or a `0600` `~/.chroxy/server-identity.json` file where no keychain is available (the same honest fallback the credential store uses). The key is minted only when encryption is on and auth is required (`--no-auth` / `--no-encrypt` carry no pinning surface). It is generated once and reused on every restart, so a returning daemon keeps the identity its clients pinned.
   ...
   > **On the keychain-less fallback path, that stability is a property of the config root.** `~/.chroxy` is the default root, not a fixed one — `CHROXY_CONFIG_DIR` relocates it (#7052, [`CONFIG.md`](../../packages/server/CONFIG.md#the-config-root-chroxy_config_dir)). Point the root somewhere that does not persist and the daemon mints a **new** identity on each start, and every client that pinned the old one refuses to connect with "server identity changed" — the failure in step 3, triggered by the operator rather than an attacker. This is not hypothetical: an unmounted container path destroying the identity key each restart is exactly what #7239 found. A relocation also leaves the previous key behind rather than moving it (`chroxy config-dir status`, #7240), so the old identity remains on disk to be read.
