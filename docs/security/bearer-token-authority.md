...
- **MCP config mutations** (`add_mcp_server` / `remove_mcp_server`, [`settings-handlers.js`](../../packages/server/src/handlers/settings-handlers.js) — `rejectMcpConfigWriteIfBound`) — a configured MCP server is a **command the daemon will spawn**, so adding one is remote-code-execution-adjacent and writes a host-owned file (`~/.claude.json`, owned by Claude Code rather than Chroxy). Removal rides the same gate: it only reduces capability, but it still mutates that host-level file, and a bound client able to delete servers could silently strip a session's tooling out from under its owner. Both are rejected with `MCP_CONFIG_FORBIDDEN_BOUND_CLIENT` (#6974), **before any validation or write** — a bound client's payload never reaches the config file. Note the contrast with `set_mcp_server_enabled` / `submit_mcp_auth_code` (#6824/#6822), which are NOT host-authority gated: they only park/unpark or authenticate a server the host already configured, so they use the weaker own-session gate. Adding a server is also still not permission to *run* it — the MCP fleet's first-use trust prompt (`requestMcpTrust`, #4457) independently gates the spawn.
