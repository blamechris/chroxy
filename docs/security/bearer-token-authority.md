...
#### Session-scoped SURVEYS reject with a degraded snapshot, not an error (#7344)

There is a third reply shape, and it is worth naming because neither of the two above describes it. `session_pr_status_request` ([`session-pr-status-handlers.js`](../../packages/server/src/handlers/session-pr-status-handlers.js)) is the first **session-scoped survey**: unlike every Control Room survey it is NOT host-gated — a bound client may legitimately ask about the session it is bound to, since the question is *"what is the state of the thing THIS session produced"*. A bound client asking about any OTHER session is refused.

The refusal is a schema-valid `session_pr_status` carrying a `reason`, not a `session_error`. That is deliberate: a client that only ever receives one reply type cannot be left waiting on a reply that never comes, which matters for a surface whose whole job is to say whether something is still in flight. Two properties make it safe:
...
Since #7436 the survey is also **rate-limited per session** (`SURVEY_MIN_INTERVAL_MS`), because #7427 made the handler mutate daemon watch state (`SessionCiWatcher.observe()`). The throttle deliberately does not distinguish token classes — a pairing-bound token is authoritative for its own session, so a per-session stamp is the right key — which makes it **cross-client visible**: one client's survey starts the window for every client of that session. A throttled request is answered by **replaying the last cached reading** (same schema, this request's `requestId`, the original `generatedAt`), so the sharing never degrades anyone's display; only a throttled request arriving before the session's first reading completes gets the schema-valid `RATE_LIMITED` reason.
