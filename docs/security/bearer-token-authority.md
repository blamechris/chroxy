...
- **Container environment lifecycle** (`destroy_environment`, [`feature-handlers.js`](../../packages/server/src/handlers/feature-handlers.js); `containers_action` with `action` `stop`/`restart`/`destroy`, [`control-room-handlers.js`](../../packages/server/src/handlers/control-room-handlers.js)) — an environment is a host container that outlives any one session, and since #7562 `destroy_environment` accepts `force: true`, which **destroys every session attached to the environment** before tearing the container down. A bound token forcing that would destroy siblings, which the rule above forbids outright, so both are rejected for a bound client: `containers_action` with a `CONTAINER_ACTION_FAILED` / `reason: 'forbidden'`, `destroy_environment` with an `environment_error` / `code: ENVIRONMENT_DESTROY_FORBIDDEN_BOUND_CLIENT` (#7571 review B1). The two arrive on different surfaces for the same operation, and only `containers_action` was gated — the destroy handler took `_client` and never read it. **That is the lesson worth keeping from this entry**: when one message gains a capability another message already has, the authority check does not come with it. Check the roster here, not the neighbouring handler you copied the policy from. Both gates run BEFORE the feature-enabled check and any id lookup, so the refusal is not an existence oracle and no session ids leak to an unauthorised caller. The other three messages on this surface — `create_environment`, `list_environments`, `get_environment` — are handled per #7576, each at the tier its capability warrants:
  ...
  - **`list_environments` / `get_environment` are REFUSED for bound clients** (#7596), with `code: ENVIRONMENT_LIST_FORBIDDEN_BOUND_CLIENT` / `ENVIRONMENT_GET_FORBIDDEN_BOUND_CLIENT`, the gate running BEFORE the feature-enabled check and any manager lookup so it is not an existence/feature-state oracle. The whole descriptor is host-level — the `sessions` roster (the "list of sibling sessions" the rule above forbids, wired #7552) plus `cwd` (an absolute host path), `image`, `containerId`, `containerUser`, `containerCliPath` — and a bound token learns nothing host-level, matching `destroy_environment` and every Control Room host survey. (#7576 briefly redacted just the `sessions` roster on the theory a bound client needed the ids for a picker; that was retired in #7596 — a bound client cannot `create_session`, so it has no picker, and the sessions-only redaction still leaked the host metadata.)
  - The three `environment_list` broadcast sites (post-create, post-destroy, and the #7552 `environment_sessions_changed` re-broadcast) fan out to **unbound (host) clients only** — a bound client is excluded on the push as on the pull. Broadcasting to bound clients would be a false-safety companion to refusing the pull: the #7552 re-broadcast fires on every session open/close, so a bound client that merely stays connected would be streamed the surface continuously. All three env gates and the broadcast filter use the fail-safe `isBoundClient` check (`environments/authority.js`) — `boundSessionId == null` is the only unbound case, so an empty-string id is treated as bound (matching the server's #4787 canonical check; a truthy check would leak on `''`).

- **Failed-restore roster + retry** (`list_failed_restores` / `retry_failed_restore`, [`session-handlers.js`](../../packages/server/src/handlers/session-handlers.js), #7625) — a session the boot restore could not bring back is parked server-side with its history and worktree preserved, and until #7625 nothing could act on it (`clearFailedRestore` had zero callers since #2954). Both messages **refuse bound clients**, with `FAILED_RESTORES_LIST_FORBIDDEN_BOUND_CLIENT` / `RETRY_FAILED_RESTORE_FORBIDDEN_BOUND_CLIENT`, and both gates run **before** any `_failedRestores` lookup so neither is an existence oracle. Two things are worth recording because the obvious reading of each is wrong:
  - **The list refuses rather than redacts**, matching `list_environments` (#7596) and for the same reason: each row carries `cwd` (an absolute host path), the provider, and the failure detail for a session the bound token is not scoped to — and the roster is itself a list of sibling sessions, which the rule above forbids a bound token from enumerating. Redacting to "ids only" would repeat the #7576 mistake that #7596 retired. The refusal is a **degraded, schema-valid `failed_restores_list`** carrying `refused: true` + `code`, not a `session_error` — the session-scoped-survey shape below, so a caller waiting on a reply never hangs. (`session_error` also carries no `code` field, so a refusal sent that way could not name its own reason.) `refused` is a separate flag from an empty `restores` array precisely so a bound client cannot read "refused" as "nothing failed".
  - **The retry stops at the UNBOUND bar, NOT strict primary**, and that is a deliberate call against the surface's first impression. It spawns a provider process and may re-enter a container, which sounds like §9 step 4 territory. But the concrete question step 4 asks is *can a caller choose a binary, argv, or environment this daemon will spawn?* — and here it cannot: the only caller-supplied value is a `sessionId`, while the provider, cwd, model and every spawn input come from the entry **the daemon itself persisted**. The daemon already performs exactly this restore at every boot with no token involved. That makes it `destroy_environment` (acts on state that already exists), not `create_environment` (caller-chosen cwd/name/limits, hence strict primary). A **user-shell** entry is refused outright (`RETRY_FORBIDDEN_USER_SHELL`), and that check lives in `SessionManager.retryFailedRestore` rather than the handler so it covers every caller — the check is class-based (`getProvider(saved.provider)?.isUserShell`, matching every other user-shell gate in that file) with the provider-name comparison kept as a fallback for a provider not registered in this process. Review corrected an earlier claim here that the state was UNREACHABLE: the daemon's own paths cannot park a user-shell session (`serializeState` skips `isUserShell`; the re-park branch requires `_isRestore`), but `restoreState()` parks whatever fails out of `~/.chroxy/session-state.json`, so a hand-edited or corrupted state file reaches it by the ordinary boot path — the same untrusted-input assumption #7082 already makes about that file. It lives in `SessionManager` rather than the handler because the strict-primary user-shell gate exists only in `handleCreateSession`, so any other caller would bypass it.
