...
| `DELETE /api/paired-devices/:id` | Live per-device revoke (#6678): drops the token from `PairingManager`'s in-memory map, so the device's next auth fails without a daemon restart — a host-level mutation beyond one session's scope. |
| `DELETE /api/paired-devices` | Live revoke-ALL — the operator panic button (#6678). Same host-level authority; every paired device must re-pair. |
