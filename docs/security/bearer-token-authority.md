...
Both revoke routes return **500 `{ "error": "revoke not persisted" }`** (not a false `ok:true` / 404) if the durable store write fails — the token stays valid and the operator retries, rather than trusting a revoke a crash would undo (#6902, §12).
