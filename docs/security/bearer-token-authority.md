...
- **`userShell.enabled` raises the blast radius of a leaked primary token.** With the embedded user-shell enabled (default OFF, #5982), a leaked primary token yields an **unmediated remote root shell** over the tunnel — not just Claude-session access (which Claude's permission engine still mediates). Operators who enable `userShell.enabled` should strongly prefer token rotation (`tokenExpiry` / `CHROXY_TOKEN_EXPIRY`) and a loopback bind. Token rotation **now severs every already-open user-shell** (#5985 — `SessionManager.destroyAllUserShellSessions` runs on the `token_rotated` event and is audited with `reason=token-rotated`), so a leaked-then-rotated primary token loses its live remote root shell immediately. **Residual:** an old token still inside its grace window could reconnect and create a *new* shell — closing that requires a current (non-grace) token to create a shell, tracked in #6004. Host-local per-session shell approval also remains future work.
