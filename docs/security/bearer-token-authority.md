...
| Revocation | Regenerate (via `chroxy init` or scheduled rotation; old token invalid after grace window), or `TokenManager.revoke()` — the panic button: old token invalid **immediately** (no grace), live user-shells severed, all connections forced to re-auth (`reason: 'revoke'`, §12, #6006) | Expiry (configurable sliding TTL, default 30d, #6598) or `PairingManager.destroy()`. Per-device revoke CLI/UI is tracked in #6599 | Session destroyed | Delete/replace the file + restart |
