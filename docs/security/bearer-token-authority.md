...
A throttled request is answered by **replaying the last cached reading** (same schema, this request's `requestId`, the original timestamp), so the sharing never degrades anyone's display; only a request arriving before the session's first reading completes gets the schema-valid `RATE_LIMITED` reason. Since #7430 only a reading that actually carries data is cached — replaying a cached *failure* would hand one transient error to every client for the whole window.
