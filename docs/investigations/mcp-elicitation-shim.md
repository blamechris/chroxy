...
- **The blockers are not technical.** They are: (a) prompt-steering hit rate is empirically poor for a tool the model has strong pretraining priors against substituting; (b) subscription-mode (Claude Max) MCP plumbing in claude TUI does not currently expose elicitation hooks the way it does for BYOK / API mode; (c) the maintenance treadmill is not eliminated — it shifts from "TUI version drift breaks the keystroke driver" to "model behavior drift breaks the steering prompt", which is equally observation-shaped and harder to debug.
