...
- **PR-2 (server surface):** `config.js` flag helper, `handlers/orchestration-handlers.js` (wired to an engine-slice stub interface: `ctx.services.orchestrator.{listRuns,getRun,startRun,respondGate,runAction}` — **planned shape, not as built**: the service is `ctx.services.orchestrationManager`, `getRun`/`respondGate` never existed, and `startRun` here took an option bag while the engine's takes a runId, which is #7138; see engine.md §12 for the real surface), `ws-message-handlers.js` registration, `ws-server.js` doc lines + `_broadcastOrchestrationDelta`, `auth_ok` capability. Tests (server `node --test`): flag-off ⇒ silent no-op; bound client ⇒ permission error; non-primary ⇒ start/approve rejected; degraded snapshot on orchestrator error; ack + `ORCHESTRATION_ACTION_FAILED` correlation echo; any test constructing SessionManager passes a temp `stateFilePath` (sandbox guard).
