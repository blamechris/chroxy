...
- Keys are saved to `~/.chroxy/credentials.json` (mode `0600`, owner-only) and are never shown again after saving — only a masked preview (last few chars). Chroxy refuses to read the file if it is more permissive than `0600`.
