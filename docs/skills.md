...
The trust ledger (`~/.chroxy/skills-trust.json`) stores path-based grant records under a `by-path` map. The JSON property names (the lookup keys) differ by platform: on macOS and Windows they are lowercased so the ledger can be queried case-insensitively, while on Linux they are written verbatim as resolved by the filesystem. The path strings stored *inside* each record (and any other field values) are not rewritten — only the property name used for lookup is platform-normalised.
