...
**Restarting the daemon with an environment-backed session (#7561):** a session created into an environment persists its binding (`environmentId` plus the resolved container) and re-enters the SAME container on restore, re-attaching to `EnvironmentInfo.sessions`. If the environment is gone, stopped, container environments have since been disabled, or the environment now runs a **rebuilt** container rather than the one the session was created in (#7619), the restore FAILS LOUDLY into the needs-attention list (`session_restore_failed`, `errorCode: ENVIRONMENT_UNAVAILABLE`) with its history preserved — it is never silently restarted outside its container, and never silently rebound to a rebuilt one.

The first three of those clear themselves: bring the environment back and the next boot restores the session. The rebuilt-container case does not — the environment is already running, and its container id will not match the persisted one again — so that session stays in the needs-attention list until the TTL prunes it (#7625 tracks the missing operator affordance).
