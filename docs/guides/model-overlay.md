...
- **Declaring an id keeps it in the picker.** On a provider whose roster is its own discovery answer (codex and the config-driven endpoints), Chroxy drops shipped-but-no-longer-reported models from the picker ([#7761](https://github.com/blamechris/chroxy/issues/7761)) — *unless* your overlay names the id. That is keyed on the entry's **presence**, whatever the entry does: a cosmetic relabel, a `contextWindow` tweak, or a `pricing`-only entry all count as "this model exists, offer it", so an entry you wrote to re-price a model will also keep that model listed once the provider retires it. Delete the entry to let it drop again. Nothing is written to the model cache on the strength of a declaration, so removing the entry takes effect on the next reload and does not survive a restart.
