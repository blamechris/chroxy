...
- `permission_request` includes an `input` field (always present, defaults to `{}`) with structured tool input for rich UI rendering; `remainingMs` (milliseconds until auto-deny) lets the client compute a local deadline without clock skew; `floored` (#7968) is the permission floor's verdict — `true` when the protected-path / secret-read floor forced this prompt to a person (see `docs/security/permission-floor.md`). Absent only from a daemon that predates #7968; read absence as unknown, never as `false`
...
- `agent_spawned` fires when the subagent tool is detected — named `Agent` on current Claude Code builds, `Task` on older ones (#7395); both are recognised, and the provider's own `task_started` is the authoritative producer because it identifies subagents by `task_type` rather than by tool name. Description truncated to 200 chars.
- `agent_completed` fires per-agent on the provider's terminal signal (`task_notification`), at turn end, or on process crash/destroy. #7340: a subagent whose backgrounding the provider CONFIRMED (`task_started` with `is_backgrounded: true`, claude-cli only) is exempt from the turn-end sweep — it outlives its turn, and `task_notification` is then its only finalizer. Because agent events are transient and never replayed from history, the server re-emits `agent_spawned` for every still-running subagent after `history_replay_end`, so a reconnect or tab switch does not empty the client's badge list.
