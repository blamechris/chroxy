...
- **Broadcast scoping (#1138):** `_broadcastToSession(sessionId, message)` defaults to `client.activeSessionId === sessionId`, delivering only to clients viewing that session. All callers use this default: `stream_delta`, `model_changed`, `permission_mode_changed`, `budget_resumed`, `primary_changed`, `session_context`, `dev_preview`, `dev_preview_stopped`, and normalizer event messages. Global messages (`session_list`, `client_joined`, `client_left`, `token_rotated`, `available_models`) use `broadcast()` instead. If a future Kanban panel needs real-time cross-session status, add a `session_status_changed` global broadcast or pass explicit `() => true` filters.
