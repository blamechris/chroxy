...
# touched packages[""]. The block below walks every lock.packages[k] where k
# starts with "packages/" and rewrites the version. Other lockfile fields
# (resolved, integrity, deps) are left untouched and will be regenerated by
# the next `npm install`.
#
# packages/server has no lockfile of its own to update here (#7324) — it is
# derived from this same root lockfile at desktop-bundle time instead
...
# now CNG (gitignored — see packages/app/.gitignore); EAS regenerates them from
# app.json at prebuild, so the native CFBundleShortVersionString / versionName
# is derived from app.json's `version` (updated above). The former manual
# ios/Chroxy/Info.plist sync was removed when iOS moved to CNG (#5642).
