...
- **Dropped redundant `chmodSync` after `writeFileSync({mode: 0o600})` (#4907 → #4917):** cosmetic cleanup with security commentary preserved — the comment block explicitly notes that `chmodSync` covers final at-rest perms but does NOT close the transient write→chmod exposure window, so the defensive pattern stays as intentional belt-and-braces.
- **Mobile `conversationIdRow` tap target ≥44pt (#4893 → #4916):** sibling fix to #4892 — adds `hitSlop` to meet Apple HIG. Coverage tests assert measured tappable area.
- **Mobile `session_stopped` strip clears on reconnect (#4909 → #4918):** follow-up to #4905 — the stale `stoppedAt`/`stoppedCode` no longer persist across a disconnect/reconnect cycle. Store now clears on `session_list` refresh.
