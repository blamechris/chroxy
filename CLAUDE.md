...
`server_status` is used for non-error status updates, while `server_error` is reserved for error conditions. `discovered_sessions` can be sent proactively when auto-discovery finds new tmux sessions (configurable via `--discovery-interval`, default 45s). `trigger_discovery` requests an immediate discovery scan. `permission_request` includes an optional `input` field with structured tool input for rich UI rendering. `user_question` forwards `AskUserQuestion` prompts from plan mode; `user_question_response` sends the user's answer back.
